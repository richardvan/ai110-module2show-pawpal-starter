...
task1b = Task(task_id="task1", pet_id="pet1", task_type=TaskType.WALK_PET,
             start_time=datetime.time(7, 14), duration_minutes=30,
             priority=Priority.HIGH, description="Morning walk", frequency=Frequency.DAILY)
task2 = Task(task_id="task2", pet_id="pet1", task_type=TaskType.FEED_FOOD,
             start_time=datetime.time(6, 0), duration_minutes=10,
...
my_pet01.tasks = [task1, task1b, task2, task3]
...
my_owner.filter_tasks(pet_name="Buddy")

schedule, warnings = my_owner.generate_schedule()
for w in warnings:
    print(w)
